...
The locked dependency set and 39 non-integration tests pass; lint, strict typing, 41 Vitest tests,
...
| Generated demo documents            | Pass   | Four deterministic, one-page DXA PDFs are visibly marked `SYNTHETIC DEMO - NOT A MEDICAL RECORD`. All pages were rendered and visually inspected for clipping, overlap, and legibility.                                                                                                                                                                                                                                                                                                                                                                                 |
...
| AWS workflow definition | Hosted pass  | The low-cost CDK path is deployed as a Python 3.12 Lambda Function URL with private KMS-encrypted S3, Secrets Manager, CloudWatch logs, and X-Ray. The larger Cognito, Step Functions, Textract, Comprehend Medical, and AgentCore definitions synthesize but are not claimed live.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Trusted retrieval       | Hosted pass  | Normalized 1,024-dimensional vectors are queried through subject-scoped CockroachDB Cloud `VECTOR(1024)` retrieval and trust filters. Hosted execution invokes Titan Text Embeddings v2, and LangChain gates retrieved IDs through the Managed MCP `select_query` allowlist.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Agent and safe actions  | Hosted pass  | A versioned safety prompt, strict schemas, medical-output policy, forced allowlisted Bedrock tool, bounded task proposal, role enforcement, idempotent review behavior, and verified memory are implemented. Hosted Lambda invokes Nova Lite through Bedrock; AgentCore remains future-ready rather than claimed live.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| No secrets in repository history                         | Pass   | Repository history and the working tree pass the secret-signature scan; secret-bearing local files are ignored. |
...
- Upload and comparison observability repair: the former longitudinal chart slot now renders a
  ...
  counterfactual statements, plus explicit links to the full Memory Trace and review queue. Upload,
...
- Phase 10: local setup, demo documents, Lambda backend, and Amplify frontend acceptance passed;
  the public README now includes the verified live architecture; Devpost submission remains
  entrant-managed.
...
- GitHub-hosted CI and the public links should be rechecked after the final submission commit and
  again before judging closes.

## README and architecture evidence

- `README.md` links directly to the hosted demo and describes the judge-visible memory proof.
- `docs/diagrams/bonetwin-architecture.svg` is a version-controlled architecture picture of the
  verified Amplify, Lambda, S3/KMS, Bedrock, LangChain/MCP, and CockroachDB Cloud path.
- The diagram explicitly separates live services from future-ready Cognito, Textract, Step
  Functions, Comprehend Medical, and AgentCore contracts so the submission makes no unsupported
  deployment claims.
